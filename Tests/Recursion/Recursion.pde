...
void makeShape(ArrayList<PVector> newShape, int shapeLoc) {
  ...
  for(int i = 0; i < shapeLoc; i++) {
    myShape.vertex(newShape.get(i).x, newShape.get(i).y);
  }
